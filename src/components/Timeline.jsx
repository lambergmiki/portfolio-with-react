...
const Timeline = () => {
      ...
      mm.add(
        {
          ...
          isNotMobile: "(min-width: 426px)",
        },
        (context) => {
          const { isMobile, isNotMobile } = context.conditions
          const tlItem = gsap.utils.toArray(".timeline-item")

          if (isNotMobile) {
            // Desktop: one scrubbed, pinned timeline
          ...
          // TODO: Fix the timeline items being cramped up on mobile. Fix animation not animating like on desktop (e.g. opacity 0 > 1, duration 0.6)
          if (isMobile) {
            // Mobile: animate items as they enter viewport
            tlItem.forEach((item) => {
              gsap.from(item, {
                y: -600,
                opacity: 0,
  ...
  return (
    ...
    <div ref={tlContainer} id="tlContainer" className="relative overflow-hidden">
      ...
      {timelineItems.map((item, i) => (
        <div
          ...
          className={`timeline-item relative w-1/2 p-6 ${
            // Does not apply to mobile screens
            i % 2 === 0
